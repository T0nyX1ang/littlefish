...
def validate_calc42(math_expr, group_id):
    ...
    try:
        if not CURRENT_42_PROBLEM_SOLVED[group_id]:
            ...
            if isinstance(expr_ast, ast.Expression):
                ...
                if abs(math_expr_value - 42.0) < 1e-6 and sorted(user_input_numbers) == CURRENT_42_PROBLEM[group_id]:
                    CURRENT_42_PROBLEM_SOLVED[group_id] = True
                    return True
        return False
...
async def calc42(session: CommandSession):
    if session.event['message_type'] == 'group':
        ...
        if validate_calc42(math_expr, group_id):
...
async def _():
    ...
    try:
        ...
        for each_group in groups:
            ...
            CURRENT_42_PROBLEM_SOLVED[group_id] = False
