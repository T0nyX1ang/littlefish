...
+ 小黑屋功能：关入小黑屋，每次发言会被机器人提醒(或禁言一段随机时间)。
...
```
    ...
    # 生成 [a, b] 之间的1个随机整数
    randi/随机数 a b
    # 生成 [a, b] 之间的c个随机整数，可重复
    randi/随机数 a b c
    # 生成 [a, b] 之间的c个随机整数，不重复
    randi/随机数 a b c 0
```

> 最多生成20个随机数，最少生成一个，如果参数`c`超出范围，将只生成一个随机数。

> 随机数生成采用`Python`自带的`random.sample`，不保证效率，但数据范围可以非常大。
...
> `GAME_FREQUENCY`代表游戏频率，其值表示小时/局，这是一个机器人运行时无法更改的变量。
...
> 即使使用了\*前缀，亦不保证精确查询能够查询到对应玩家，因为最大查询的用户数为`10`名。
...
+ 小黑屋功能中支持**禁言**用户，有`1%`概率被禁言`1-3`天，有`9%`概率被禁言`1-24`小时，有`90%`概率被禁言`1-60`分钟。

> 机器人无法禁言管理员，此时会触发正常的小黑屋指令。

+ 支持解析加群申请，将加群用户联萌ID信息公开。
