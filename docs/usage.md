...
## 42点
+ 算42点小游戏：每日8-23点42:42自动触发，广播至小鱼加入的**所有**群中。
...
+ 42点游戏规则(暂定): 
    - 每日某些小时的42分42秒, 会给出5个位于0至13之间的整数，玩家需要将这五个整数(可以调换顺序)通过四则运算与括号相连，使得结果等于42(有`0.25%`的概率随机为`24-60`中的某个数);
...
> 有关手动生成/取消42点: 任何人都可以在42点开始以前生成题目，只有生成题目的玩家可以取消42点题目，如果是每小时固定的42点，无人(包括超级用户)可以取消该项目。手动生成的42点计算得分，但不加入玩家总分中。
...
## 推送功能
+ B站直播推送：小鱼会每隔20秒查询订阅用户名单中的所有用户，如果某个用户处于直播状态，小鱼将会发送TA的直播间地址。

```
    subscribe/订阅用户 +/- 用户B站UID
```

> 该功能采用轮询方式，可能会相当耗费流量，同时查询有被风控的风险，请谨慎决定其开启状态。
