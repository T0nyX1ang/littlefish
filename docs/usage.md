...
> 机器人于每天03:00:00每隔4小时自动获取一次群员信息。有**新用户**加群时，机器人也会主动获取群员信息。即使群员退群，**其信息也会保留**。
...
> 屏蔽词汇仅用在机器人复读时生效，其功能为: 语句中含有屏蔽词，则机器人不会复读。**不推荐设置过多的屏蔽词，可能会导致性能问题。**
...
> (增量)复读功能有5%的概率(默认值，群管理员可以调整)被机器人强行打断，此时会重置相应的复读计数器。如果复读的句子前两个字是`打断`，机器人会自动在句子前加上一个`打断`。

> (增量)复读功能有5%的概率(默认值，群管理员可以调整)被机器人随机倒装句子中的某一部分/随机替换句子中的某个QQ表情，此时会重置相应的复读计数器，增量复读部分参与这个功能。
