...
-   经典模式排行榜查询：包含用户的`排名`，`昵称`，`ID`与`成绩`。 :material-circle:{.green_circle}
    ...
    === "中文指令"
    === "时间"
    `tex 排名 时间 $查询页码$ 全部/盲扫/标旗 全部/初级/中级/高级 `
    ...
    === "英文指令"
    === "时间"
    `tex rank time $page$ all/nf/fl all/beg/int/exp `
...
-   设置复读参数：设置复读的`变形概率`(0-100 整数)与`打断概率`(0-100 整数)。 :material-circle:{.red_circle}
    ...
    === "英文指令"
    `tex repeaterparam $mutate-prob$ $cut-in-prob$ `
