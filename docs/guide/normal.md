...
+   超级用户才可触发的权限或广播权限 :material-circle:{.red_circle}
...
## 群组管理功能
+   解析加群申请：将加群用户联萌ID信息公开，并**自动拒绝**不符合加群条件的用户。 :material-circle:{.yellow_circle}
...
+   自动更新群成员信息: 小鱼在每天03:00:00每隔4小时自动获取一次群员信息，误差为30秒。 :material-circle:{.red_circle}
...
+   自动推送联萌每日一星：于每天00:01:30进行推送，误差为30秒，该功能自动触发，广播至小鱼加入的**所有**群中。 :material-circle:{.red_circle}
...
+   自动推送每日一图：每天00:03:00进行推送，误差为30秒，该功能自动触发，广播至小鱼加入的**所有**群中。 :material-circle:{.red_circle}

!!! tip
    如果每日一图尚未完成，`第一名成绩`会显示为 `inf`。

### 用户扫雷数据
+   按ID查询联萌用户扫雷相关数据：获得用户的`时间记录(含排名)`，`bvs记录(含排名)`，`游戏局数(含开率)`，`当前等级`与`当前排名`。 :material-circle:{.green_circle}
```tex
    id/联萌 用户ID
```

+   个人信息查询：本功能需要群主给以查询用户ID命名的头衔，查询结果与上文一致。 :material-circle:{.green_circle}
```tex
...
+   自动推送联萌每个等级的人数与总计人数：每周一00:00:00进行推送，误差为30秒，该功能自动触发，广播至小鱼加入的**所有**群中。 :material-circle:{.red_circle}
...
+   自动推送对战机器人状态：每天00:00:00进行推送，误差为30秒，该功能自动触发，广播至小鱼加入的**所有**群中。 :material-circle:{.red_circle}
...
+   设置复读参数：设置复读的`变形概率`与`打断概率`。 :material-circle:{.red_circle}
```tex
...
+   小黑屋功能：在小鱼为管理员时，禁言进入小黑屋的用户一段时间。 :material-circle:{.yellow_circle}
```tex
    # 进入自定义时间段的小黑屋
...
+   随机数功能: 生成可重复/不可重复的随机数。 :material-circle:{.green_circle}
```tex
...
### 规则及说明
+   算42点小游戏：每日8-23点42:42自动触发，广播至小鱼加入的**所有**群中。 :material-circle:{.red_circle}
...
+   关于得分的说明:
    ...
    +   首杀玩家额外加5分，最后一个解玩家额外加 $10 \times n^{2-m} \times (1 - \frac{1}{N})$ 分，其中 $N$ 为总解数;
...
### 相关指令
+   回答42点题目。 :material-circle:{.green_circle}
```tex
    calc42/42点
```

+   显示42点规则。 :material-circle:{.green_circle}
```tex
    guide42/42点说明
```

+   显示42点游戏得分。 :material-circle:{.green_circle}
```tex
    score42/42点积分/42点得分
```

+   显示42点游戏排行。 :material-circle:{.green_circle}
```tex
    rank42/42点排名/42点排行
```

+   显示42点游戏今日得分排行。 :material-circle:{.green_circle}
```tex
    dailyrank42/42点今日排名/42点今日排行
```

+   手动生成42点(或其它点数)题目。 :material-circle:{.yellow_circle}
```tex
    # 生成42点
    manual42 +
...
+   手动取消42点题目。 :material-circle:{.yellow_circle}
```tex
    manual42 -
```
...
## 其它功能
+   B站直播推送：小鱼每隔20秒查询订阅用户名单中的所有用户，如果某个用户处于直播状态，小鱼将会发送TA的直播间地址。 :material-circle:{.red_circle}
```tex
    # 根据B站UID更新订阅列表
...
+   自动写入数据库：小鱼会从00:30:00开始，每隔2小时将当前状态写入本地数据库。 :material-circle:{.red_circle}
