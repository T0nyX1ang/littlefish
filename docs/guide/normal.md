...
+   小鱼为管理员才能触发的权限 <Badge text="#" type="warning"/>
...
## 用户信息查询
+   按ID搜索联萌用户：获得用户的`时间记录(含排名)`，`bvs记录(含排名)`，`游戏局数(含开率)`，`当前等级`与`当前排名`。 <Badge text="$" type="tip"/>
...
+   录像数据解析功能：包含雷图的信息以及用户的操作信息，基本与电脑端`arbiter`的计数器的统计信息保持一致。 <Badge text="$" type="tip"/>
...
## 推送功能
+   B站直播推送：小鱼会每隔20秒查询订阅用户名单中的所有用户，如果某个用户处于直播状态，小鱼将会发送TA的直播间地址。 <Badge text="!" type="error"/>
...
+   加油功能：包含了少量的加油形式和随机的加油语句。 <Badge text="$" type="tip"/>
```tex
    # 空泛加油
    cheer/加油
...
小鱼有5%的概率强行打断复读，此时会重置相应的复读计数器。如果复读的句子前两个字是`打断`，小鱼会自动在句子前加上一个`打断`。

小鱼有5%的概率随机倒装句子中的某一部分/随机替换句子中的某个QQ表情，此时会重置相应的复读计数器，增量复读部分参与这个功能。
...
## 存档
+   将小鱼的当前状态写入本地数据库。 <Badge text="!" type="error"/>
```tex
    save/存档
```
